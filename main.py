...
def parse_nhsjobs(soup: BeautifulSoup, base: str) -> list[dict]:
    ...
    Job links follow /candidate/jobadvert/<id>.
    needs_detail=True so we visit each advert page and read the real <h1> title
    instead of relying on the short listing-page link text.
    """
    seen_ids: set = set()
    jobs = []
    for a in soup.select("a[href*='/candidate/jobadvert/']"):
        ...
        text = a.get_text(strip=True)
        if text and len(text) > 5:
            jobs.append({"title": text, "link": href, "needs_detail": True})  # ← was False
    return jobs
